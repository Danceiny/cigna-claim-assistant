...
- uTools 插件：作为桌面入口选择或拖入报销目录，或直接拖入一组 PDF/图片做精确文件扫描，生成提交计划，并打开 Cigna/Chrome 提交入口。
...
uTools 插件里的 `扫描时压缩超限 PDF` 会传给同一套本地扫描 CLI，适合在桌面端先生成带压缩结果的计划。CLI 也支持重复传入 `--file /path/a.pdf --file /path/b.pdf`，只扫描指定文件，不会扩大到父目录。
