...
- uTools 插件：作为桌面入口固定一个报销收件文件夹，之后只要把 PDF/图片扔进该文件夹再扫描；也兼容直接拖入一组文件，生成提交计划，并打开 Cigna/Chrome 提交入口。
...
- 长期病症最早治疗日期不用每次手填；它只是可选兜底值。工具会优先从 PDF/OCR 文本或文件行修正里识别最早治疗日期，只有整单都识别不到时才使用这里的兜底值。
- 可在 popup 中对单个文件修正类型、服务日期、最早治疗日期；适合扫描件没有文本层或文件名不规范时补救。
- popup 会为 PDF 第一页和图片生成本地缩略图，方便修正扫描件类型和日期。
- 最早提交服务日期也是可选过滤条件，不填则不限制服务日期；如果只想提交某天之后的材料，例如 `2026-05-08` 以后，再填写它，早于该日期的分组会阻塞。
- uTools 支持固定一个收件文件夹作为主入口：之后只要把 PDF/图片扔进该文件夹，再点击扫描即可。打开 `只扫描新增或变化文件` 时，uTools 会保存目录快照，后续只把新增、内容变化或补了 OCR sidecar 的文件交给扫描器。第一次接管已有历史文件夹时，可先点 `建立当前目录基线`，之后再丢新文件。打开 `扫描后自动整理命名副本` 时，会把识别后的文件按 `服务日期/服务日期_上传顺序_类型_原文件名.pdf` 复制到 `outputs/organized-claims/`，原文件不改名。
- 一天一单自动分组。
...
- 防重：
  - uTools 目录快照只负责“这个收件文件夹里哪些文件已经见过”，避免每次全量处理几十个历史文件；它不是 Cigna 提交成功账本。
  - SHA-256 文件哈希去重。
...
3. 设置被保险人、报销理由、治疗国家/地区、理赔类型、就诊类型、付款账户关键词，以及是否允许提交需复核但未阻塞的项目。这些设置会保存；报销理由没有内置默认值，必须明确填写或从备份导入；默认允许提交普通扫描件类 `review` 项。长期病症最早治疗日期和最早提交服务日期都可留空：前者只是最早治疗日期识别失败时的兜底，后者只是提交日期范围过滤。
...
uTools 插件里的 `扫描时压缩超限 PDF` 会传给同一套本地扫描 CLI，适合在桌面端先生成带压缩结果的计划。CLI 也支持重复传入 `--file /path/a.pdf --file /path/b.pdf`，只扫描指定文件，不会扩大到父目录。更推荐固定文件夹扫描；需要整理混乱文件名时可加 `--organize`，默认输出到 `outputs/organized-claims/`。
