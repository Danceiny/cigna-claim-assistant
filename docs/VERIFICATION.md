...
- uTools 形态支持选择或拖入报销文件夹，也支持直接拖入一组 PDF/图片做精确文件扫描；拖入文件时不会扩大到父目录。
...
- 用户可在真实 Cigna 页面执行彩排，验证表单填写、日期选择器、附件上传和付款选择，并在最终提交前停止。
...
- 源码仓库一键 release 验证：
  ...
  - 顺序覆盖核心识别、核心同步、本地 helper、扫描 CLI、PDF 压缩、mock Cigna 提交、Chrome background/popup/manifest、release 打包、打包后 Chrome 扩展加载、uTools `.upx` 审计、uTools 页面 smoke、release helper 解包运行和 release 解压后用户侧入口冒烟验证。其中扩展加载和页面 smoke 可能启动 Playwright/Chromium；本机默认拒绝运行，除非显式设置 `ALLOW_LOCAL_BROWSER_VERIFY=1`，通常应交给 GitHub Actions 远端执行。
...
- Cigna mock 页面端到端提交：
  ...
  - 覆盖被保险人选择、国家/地区、理赔类型、就诊类型、日期选择器点击、诊断填写、文件上传、上传完成文案变体、上传确认、付款账户选择、真实页面彩排停在最终检查页且不勾免责声明/不提交、免责声明勾选、提交 ID 提取，以及成功页缺少提交 ID 时仍记录为已提交并标记 `submissionIdMissing`。
- background worker：
...
- 覆盖设置持久化、必填校验、独立助手页打开和加载、侧边栏窄宽度无横向溢出、文件选择、文件夹拖拽递归读取、OCR sidecar 进入扩展识别但不进入附件、拖入/选择后自动处理并提交开关、文件移除、清空当前批次、PDF 缩略图、文件级类型/日期修正、追加文件后保留已有人工修正、计划导出、Cigna 页面诊断导出、提交预检导出且不包含文件 base64、真实页面彩排消息带 `dryRun` 且不写入防重账本、提交前批次快照、提交预检和实际提交共享 `submissionFingerprint`、账本导入导出、完整备份导入导出、正在提交时拒绝重复启动、旧提交锁过期恢复、review 跳过策略、压缩后再提交、提交后按账本刷新防重。
- manifest:
...
- 真实页面彩排会上传附件到 Cigna 页面，只是不执行最终提交；仍应使用低风险材料先验收。
