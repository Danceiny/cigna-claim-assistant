...
assert.equal(releaseManifest.capabilities.includes("live-cigna-dry-run"), true);
...
assert.match(utoolsPreload, /organizeEnabled/);
assert.match(utoolsPreload, /onlyNewEnabled/);
assert.match(utoolsPreload, /saveFolderState/);
assert.match(utoolsPreload, /folderStateKey/);
...
assert.match(utoolsHtml, /id="onlyNewEnabled"/);
assert.match(utoolsHtml, /id="baselineDir"/);
...
assert.match(popupHtml, /拖入 PDF\/图片\/文件夹/);
assert.match(assistantHtml, /拖入 PDF\/图片\/文件夹/);
assert.match(popupHtml, /id="dryRunSubmit"/);
...
assert.match(popupJs, /真实页面彩排/);
assert.match(popupJs, /dry-run-ready/);
...
assert.match(background, /dry-running/);
assert.match(background, /dry-run-ready/);
...
assert.match(scanClaims, /arrayArg\(args\.file\)/);
assert.match(scanClaims, /collectInputFiles/);
...
assert.match(readme, /--file \/path\/a\.pdf/);
...
assert.match(install, /只扫描这些指定文件/);
assert.match(verification, /未在真实登录态 Cigna Envoy/);
assert.match(verification, /精确文件扫描/);
assert.doesNotMatch(verification, /拖入单个 PDF 时使用它所在目录/);
